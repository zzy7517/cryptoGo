...
export default function ContractData({ fundingRate, openInterest, loading }: ContractDataProps) {
  ...
  const getFundingRateStatus = (rate: number | null): { color: string; label: string } => {
    if (rate === null) return { color: 'text-gray-600', label: '无数据' };
    if (rate > 0.0005) return { color: 'text-red-600', label: '多头费用高' };
    if (rate < -0.0005) return { color: 'text-green-600', label: '空头费用高' };
    return { color: 'text-gray-700', label: '费率正常' };
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
        <h3 className="text-sm font-medium text-gray-700 mb-4">资金费率</h3>
        {fundingRate?.funding_rate !== null ? (
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-gray-600 text-sm">当前费率</span>
              <span
                className={`font-bold text-lg ${
                  (fundingRate?.funding_rate || 0) >= 0 ? 'text-red-600' : 'text-green-600'
                }`}
              >
            ...
            <div className="flex justify-between items-center">
              <span className="text-gray-600 text-sm">下次结算</span>
              <span className="font-bold text-gray-800">{formatTime(fundingRate?.next_funding_time)}</span>
            </div>
            <div className="text-xs mt-2">
              <span className={`${fundingStatus.color} font-medium`}>● {fundingStatus.label}</span>
            </div>
      ...
      {/* 持仓量 */}
      <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
        <h3 className="text-sm font-medium text-gray-700 mb-4">持仓量</h3>
        {openInterest?.open_interest !== null ? (
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-gray-600 text-sm">总持仓量</span>
